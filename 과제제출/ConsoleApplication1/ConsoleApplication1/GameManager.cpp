...
void GameManager::SettingGame(int iSelect)
{
	...
	m_DrawManager.MapDraw(m_iWidth, m_iHeight);
	SetCursor();
	m_bGameOver = false;
}
...
{
	char buf[256];
	sprintf(buf, "mode con: lines=%d cols=%d", m_iHeight + 7, m_iWidth * 2);
	system(buf);
}
...
void GameManager::Playing()
{
	while (!m_bGameOver)
	{
		m_DrawManager.DrawPoint(m_Cursor.strCursor, m_Cursor.ix, m_Cursor.iy);
		char ch = getch();
		switch (ch)
		{
		case KEY_LEFT:
		...
		case KEY_ENTER:
			...
			break;
		case KEY_FLAG:
			m_FlagManager.SettingFlag(m_Cursor.ix, m_Cursor.iy);
			break;
		case KEY_OPTION:
			NewGame();
			Delete();
