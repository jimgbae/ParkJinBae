...
{
	char buf[256];
	sprintf(buf, "mode con: lines=%d cols=%d", m_iHeight *3, m_iWidth * 7);
	system(buf);
}
...
void GameManager::Playing()
{
	...
	while (!m_bGameOver)
	{
		...
		{
			m_FlagManager.MakeFlag(m_iWidth,m_iHeight);
		}
	}
}
...
void GameManager::SelectCourse()
{
	...
	m_DrawManager.DrawMidText("VeryHard : 4", m_iWidth, m_iHeight*0.4f + 3);
	m_DrawManager.DrawMidText("입력 : ", m_iWidth, m_iHeight*0.4f + 4);
	cin >> iSelect;
	switch (iSelect)
	{
	case COURSE_EASY:
...
void GameManager::SetCourse(int iSelect)
{
	...
	case COURSE_HARD:
		...
		break;
	case COURSE_VERY_HARD:
		m_iWidth = VERY_HARD_WIDTH;
		m_iHeight = VERY_HARD_HEIGHT;
		m_iDefault = COURSE_VERY_HARD;
		m_MineManager.SettingMineList(VERY_HARD_MINE);
		m_FlagManager.SettingFlagList(VERY_HARD_MINE);
		break;
	}
}
