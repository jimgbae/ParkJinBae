...
void MineManager::SearchMine()
{
	for (int i = 0; i < m_iMineList; i++)
	{
		...
		tmp->GetNode(m_Mine[i].GetX(),m_Mine[i].GetY(), NULL);
		if (m_SaveMine == NULL)
			m_SaveMine = tmp;
		else
		{
			SaveMine* Ntmp = m_SaveMine;
			while (Ntmp->GetLink() != NULL)
				Ntmp = Ntmp->GetLink();
			Ntmp->SaveLink(tmp);
		}
	}
...
void MineManager::ShowSaveMine(int Width, int Height)
{
	...
	while (tmp != NULL)
	{
		m_DrawManager.DrawPoint(to_string(icount++)+"¹ø Mine x : " + to_string(tmp->GetX()) + " y : " + to_string(tmp->GetY()),x+1, y++);
		tmp = tmp->GetLink();
		if (y == Height)
		{
			y = 0;
