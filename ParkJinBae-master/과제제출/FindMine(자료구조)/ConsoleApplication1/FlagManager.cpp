...
{
	if (tmp->GetLink() != NULL)
		CheckFlag(tmp->GetLink());
	m_DrawManager.DrawFlag(tmp->GetX(), tmp->GetY());
}
...
void FlagManager::ResetFlag()
{
	...
	else
	{
		...
		if ((*m_pSaveFlag)->GetLink() == NULL)
		{
			*m_pSaveFlag = tmp->GetLink();
			delete tmp;
			tmp = NULL;
		}
		else
